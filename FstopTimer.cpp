...
FstopTimer::FstopTimer(Adafruit_SSD1306 &l, SMSKeypad &k, RotaryEncoder &r,
                       ButtonDebounce &b,
                       ButtonDebounce &fs,
                       LEDDriver &led,
  ...
  : disp(l), keys(k), rotary(r), button(b), footswitch(fs), leddriver(led), tsl(t),
    ...
    deckey(keys), comms(l),
    expctx(&inbuf[0], 1, 2, &disp, 0, 24, true),
    gradectx(&inbuf[0], 3, 0, &disp, 50, 16, false),
    stepctx(&inbuf[0], 1, 2, &disp, 0, 16, false),
    dryctx(&inbuf[0], 0, 2, &disp, 0, 16, false),
    intctx(&inbuf[0], 1, 0, &disp, 0, 16, false),
    paperctx(&inbuf[0], 1, 0, &disp, 0, 16, false),
